...
for n=1:length(folders)
    ...
    if (fold(1)~='.')&&(fold(1)~='P')
        bin=bin+1;
        load(['../Data/Results/' fold '/iCOH_FULL'], 'globalSyncXP')
        M=globalSyncXP;
        load(['../Data/Results/' fold '/iCOH_FULL_BL'], 'globalSyncXP')
        BL=globalSyncXP;
        for freqStim=1:2
            for cond=1:2
...
%         [a b c d]=ttest(squeeze(Z(:,cond1,stim,freqA,:,:)),zeros(size(squeeze(B(:,cond1,stim,freqA,:,:)))));
...
%         
%         subplot(size(freqsBeat,2),3,bin)
%         [a b c d]=ttest(squeeze(Z(:,cond2,stim,freqA,:,:)),zeros(size(squeeze(B(:,cond2,stim,freqA,:,:)))));
%         BBT_Toposync(reshape(d.tstat,64,64),0,1:64,1:64,3)
%         colormap(redblue(256))
...
%         
%         subplot(size(freqsBeat,2),3,bin)
%         [a b c d]=ttest(squeeze(Z(:,cond1,stim,freqA,:,:)),squeeze(Z(:,cond2,stim,freqA,:,:)));
%         BBT_Toposync(reshape(d.tstat,64,64),0,1:64,1:64,3)
%         colormap(redblue(256))
...
for stim = 1:2
    for freqA = 1:7
        display(' ')
        display(' ')
        display(['iCOH ',conditions{cond1},' vs. ',conditions{cond2},' Stim=',freqsStim{stim},' Freq=',freqAnal{freqA}])

        [results] = stat_LENA2(rmdiag(squeeze(Z(:,cond1,stim,freqA,:,:))),rmdiag(squeeze(Z(:,cond2,stim,freqA,:,:))),optionsbiv);
        ...
        if length(PosCl)>0
            ...
            for idx=1:length(PosCl)
                ...
                title(['iCOH ','Positive Cluster ?',conditions{cond1},' vs. ',conditions{cond2},' Stim=',freqsStim{stim},' Freq=',freqAnal{freqA}])
            end
        end
        ...
        if length(NegCl)>0
            ...
            for idx=1:length(NegCl)
                ...
                figure()
                BBT_Toposync(reshape(tmps,64,64),5,1:64,sum(reshape(tmps,64,64)+reshape(tmps,64,64)')<0,0.5)
                title(['iCOH ','Negative Cluster ?',conditions{cond1},' vs. ',conditions{cond2},' Stim=',freqsStim{stim},' Freq=',freqAnal{freqA}])
            end
        end
        
        display(' ')
        display(['iCOH ',conditions{cond1},' vs. BL Stim=',freqsStim{stim},' Freq=',freqAnal{freqA}])

        [results] = stat_LENA2(rmdiag(squeeze(Z(:,cond1,stim,freqA,:,:))),zeros(size(squeeze(B(:,cond1,stim,freqA,:,:)))),optionsbiv);
        ...
        if length(PosCl)>0
            ...
            for idx=1:length(PosCl)
                ...
                figure()
                BBT_Toposync(reshape(tmps,64,64),5,1:64,sum(reshape(tmps,64,64)+reshape(tmps,64,64)')>0,0.5)
                title(['iCOH ','Positive Cluster ?',conditions{cond1},' vs. BL Stim=',freqsStim{stim},' Freq=',freqAnal{freqA}])
            end
        end
        ...
        if length(NegCl)>0
            ...
            for idx=1:length(NegCl)
                ...
                figure()
                BBT_Toposync(reshape(tmps,64,64),5,1:64,sum(reshape(tmps,64,64)+reshape(tmps,64,64)')<0,0.5)
                title(['iCOH ','Negative Cluster ?',conditions{cond1},' vs. BL Stim=',freqsStim{stim},' Freq=',freqAnal{freqA}])
            end
        end

        
        display(' ')
        display(['iCOH ',conditions{cond2},' vs. BL Stim=',freqsStim{stim},' Freq=',freqAnal{freqA}])

        [results] = stat_LENA2(rmdiag(squeeze(Z(:,cond2,stim,freqA,:,:))),zeros(size(squeeze(B(:,cond2,stim,freqA,:,:)))),optionsbiv);
        ...
        if length(PosCl)>0
            ...
            for idx=1:length(PosCl)
                ...
                figure()
                BBT_Toposync(reshape(tmps,64,64),5,1:64,sum(reshape(tmps,64,64)+reshape(tmps,64,64)')>0,0.5)
                title(['iCOH ','Positive Cluster ?',conditions{cond2},' vs. BL Stim=',freqsStim{stim},' Freq=',freqAnal{freqA}])
            end
        end
        ...
        if length(NegCl)>0
            ...
            for idx=1:length(NegCl)
                ...
                figure()
                BBT_Toposync(reshape(tmps,64,64),5,1:64,sum(reshape(tmps,64,64)+reshape(tmps,64,64)')<0,0.5)
                title(['iCOH ','Negative Cluster ?',conditions{cond2},' vs. BL Stim=',freqsStim{stim},' Freq=',freqAnal{freqA}])
            end
        end
    end
end