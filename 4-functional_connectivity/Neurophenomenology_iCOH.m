...
for freqA = 1:7
    ...
    if length(PosCl)>0
        ...
        for idx=1:length(PosCl)
            ...
            colormap(redblue(256))
            title(['iCOH ','Positive Cluster ? MR Freq=',freqAnal{freqA}])
        end
    end
    ...
    if length(NegCl)>0
        ...
        for idx=1:length(NegCl)
            ...
            BBT_Toposync(diag(results.T_orig_ori),5,1:64,tmps==-1,1)
            colormap(redblue(256))
            title(['iCOH ','Negative Cluster ? MR Freq=',freqAnal{freqA}])
        end
    end
end
...
for freqA = 1:7
    ...
    if length(PosCl)>0
        ...
        for idx=1:length(PosCl)
            ...
            BBT_Toposync(diag(results.T_orig_ori),5,1:64,tmps==1,1)
            colormap(redblue(256))
            title(['iCOH ','Positive Cluster ? AD Freq=',freqAnal{freqA}])
        end
    end
    ...
    if length(NegCl)>0
        ...
        for idx=1:length(NegCl)
            ...
            BBT_Toposync(diag(results.T_orig_ori),5,1:64,tmps==-1,1)
            colormap(redblue(256))
            title(['iCOH ','Negative Cluster ? AD Freq=',freqAnal{freqA}])
        end
    end
end
